...
public class Comment extends JPanel {
    ...
    public Comment(Model.Comment c){
        ...
        JLabel author = new JLabel(c.getUser().getName(),20,GUIConstants.post,Font.BOLD);
        header.add(author, BorderLayout.WEST);
        add(header);

        JPanel center = new JPanel(new FlowLayout(FlowLayout.LEADING));
        center.setBackground(null);
        JTextArea content = new JTextArea(c.getContent(),18,GUIConstants.post,Font.PLAIN);
        center.add(content);
        add(center);

        JPanel bottom = new JPanel(new BorderLayout());
        bottom.setBackground(null);
        bottom.add(new JLabel(c.getDateToString(),15,GUIConstants.post,Font.PLAIN),BorderLayout.EAST);
        add(bottom);
